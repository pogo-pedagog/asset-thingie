...
const USE_REMOTE_IMAGES_KEY = "at_checkpoints_use_remote_images";
...
export const useAssetsStore = defineStore("at-checkpoints-assets", () => {
  ...
  async function loadFilters(): Promise<void> {
    try {
      ...
      filterOptions.value = await api.fetchFilters("Checkpoint", undefined, tagsForFacet);
    } catch {
  ...
  async function loadSubfolders(): Promise<void> {
    ...
    try {
      const res = await api.fetchSubfolders(folderPath.value || undefined, "checkpoint");
  ...
  function assetQueryBody() {
    ...
    return {
      ...
      content_type: "Checkpoint" as const,
