...
/** Directory path + Comfy filename when both exist (no duplicate suffix). */
const pathDisplay = computed(() => {
  const d = detail.value;
  if (!d) return "";
  const p = (d.path ?? "").trim();
  const f = (d.comfy_lora_name ?? "").trim();
  if (!p && !f) return "";
  if (!f) return p;
  if (!p) return f;
...
function openExampleLightbox(ex: ExampleMediaItem): void {
  ...
  if (mt === "video" && play) {
    ...
    // Poster / still only (no local video); ``ex.url`` is often the poster JPEG.
    ...
    <div class="at-detail__panel" @click.stop>
      ...
      <div v-else-if="detail" class="at-detail__scroll">
        ...
        <div class="at-detail__syntax">
          <code class="at-detail__code">{{ detail.lora_syntax || "—" }}</code>
        </div>
        <p
