...
const { connected, items, loading, loadingMore, viewMode, settingsOpen, selectedAssetId } = storeToRefs(store);

/** Scrollport for the asset list (`overflow: auto`). IntersectionObserver is unreliable here vs nested overflow / iframe. */
...
/** After the first page (or filter change), prefetch if the pane is still “short” — no scroll event yet. */
watch(
...
<template>
  <div class="at-app">
    ...
    <div v-else class="at-main-column">
      <!-- One chrome block when connected so an empty search result doesn’t remount Toolbar (which would drop search focus). -->
      <div class="at-chrome">
        <Toolbar />
        <StatusBar />
      </div>
      ...
      >
        <p>No LoRAs in the index for this filter.</p>
